...
| 団体名 | URL | 分野 | 取得方式 | 件数 |
|--------|-----|------|----------|------|
| 公益社団法人 地盤工学会 | jiban.or.jp | 地盤・基礎 | HTML 直接取得 | 5 |
| 一般財団法人 日本建築センター | bcj.or.jp | 構造・法規 | HTML 直接取得 | 3 |
| 公益社団法人 日本建築学会 | aij.or.jp | 構造 | HTML 直接取得 | 1 |

### 取得困難・除外記録

| 団体/機関 | URL | 困難理由 | 分類 |
|-----------|-----|----------|------|
| 日本補償コンサルタント協会 | jcca-net.or.jp | 詳細情報が PDF のみ、HTML 概要のみ | PDF 依存 |
| 公益社団法人 日本測量協会 | jsurvey.jp | Shift-JIS エンコード → 取得時に文字化け | 文字コード問題 |
| 全国地質調査業協会連合会 | zenchiren.or.jp | 講習詳細が別ページに分散・構造不安定 | 構造分散 |
| 関東地方整備局（DX 研修） | ktr.mlit.go.jp | 研修一覧が PDF ファイルのみ | PDF 依存 |
| 国土交通省本体 | mlit.go.jp | 研修情報が部署別に分散・職員向けが多い | 情報分散・対象外 |
| 建設コンサルタンツ協会 | jcca.or.jp | 研修コンテンツ少・CM/PM 特化 | コンテンツ薄 |
| 公益社団法人 土木学会 | jsce.jp | 支部分散・最新詳細ページが少ない | 構造分散 |

### 観察メモ

- 日本建築学会は `robots.txt` で `claudebot` を明示的に制限なしとしている
- 地盤工学会・日本建築センターは静的 HTML で費用・日時・申込期限が明記されており AI 取得適性が高い
- 補償・測量分野は他分野より情報公開インフラが遅れている可能性が示唆される（PDF 依存・文字コード問題）
- 国交省本体の研修情報は部署別・目的別に分散しており索引化が困難（= 情報公開構造の観測データ）
- `robots.txt` を設置していない団体が多い（mlit.go.jp、jsurvey.jp、jcca.or.jp 等）→ 設置の有無自体も観測対象
